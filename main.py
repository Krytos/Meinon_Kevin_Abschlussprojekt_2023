...
if compare_start_date > compare_end_date or (compare_start_date or compare_end_date) > datetime.date(
		datetime.now()
		) or compare_start_date > start_date or compare_end_date > end_date:
	st.sidebar.error("Invalid Date")
...
month_name = f"{datetime(compare_start_date.year, compare_max_month, 1).strftime('%B')} - {datetime(end_date.year, max_month, 1).strftime('%B')}" if compare else datetime(
...
def main():
	...
	dimensions = ["city", "PagePath", "landingPage", "sessionDefaultChannelGroup", "country", "language",
	              "deviceCategory", "date"]
	...
	for dimension in dimensions:
		...
		if compare:
			...
			if dimension == "date":
				...
				df_date_compare = pd.DataFrame(data_compare, columns=[dimension] + [metric.name for metric in metrics])
		...
		for row in response.rows:
			if dimension == "date":
				...
				data.append(
					[value.value for value in row.dimension_values] + [round(float(value.value), 2) for value in
					                                                   row.metric_values]
					)
	...
	with st.container():
		...
		st.subheader("c) Visualisierung ")
		st.bar_chart(df_country.head(10), y=["totalUsers", "newUsers"], x="country", use_container_width=True)
		st.bar_chart(df_city.head(10), y=["totalUsers", "newUsers"], x="city", use_container_width=True)
		st.bar_chart(df_language.head(10), y=["totalUsers", "newUsers"], x="language", use_container_width=True)
	...
	with st.container():
		...
		st.bar_chart(df_page.head(10), y=["screenPageViews"], x="PagePath", use_container_width=True)
	...
	with st.container():
		...
		# print(df_channel)
		st.subheader("c) Visualisierung ")
		st.bar_chart(df_channel, y=["sessions"], x="sessionDefaultChannelGroup", use_container_width=True)
